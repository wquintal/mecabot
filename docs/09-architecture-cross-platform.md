...
| **Diffusion / licence** | **Non décidée** | Une seule contrainte qui préserve toutes les options : **aucune dépendance GPL**. Depuis le 2026-08-26 ce n'est plus une phrase mais une porte de CI en liste d'autorisation — `deny.toml`. Voir `11` §7. |
| **Relecture du diff** | **Trois relecteurs de code, pas de porte humaine** | Gemini, CodeRabbit, Claude — plus `cargo-deny`, qui est une porte de dépendances et non un relecteur. ⛔ Règle valide **tant que la flotte tourne** : le plancher est de trois relecteurs de code indépendants, soit exactement l'état actuel. Décidé le 2026-08-26, voir `12` §6.3. |
