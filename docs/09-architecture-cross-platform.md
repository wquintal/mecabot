...
| **Relecture du diff** | **Trois relecteurs de code, pas de porte humaine** | Gemini, CodeRabbit, Claude — plus `cargo-deny`, qui est une porte de dépendances et non un relecteur. ⛔ Règle valide **tant que la flotte tourne** : le plancher est de trois relecteurs de code indépendants sur une branche interne, deux plus une lecture humaine sur un fork. ⚠️ **Au 2026-08-27 le plancher n'est pas tenu** — un seul relecteur actif sur trois (Gemini pas installé ici, Claude tombé sur sa clé d'API), donc la porte humaine reste requise. Décidé le 2026-08-26, mesuré le 2026-08-27, voir `12` §6.3. |
