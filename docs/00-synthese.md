...
| **`12-standards-de-developpement.md`** | **Comment le développement est structuré : ce qui est emprunté à punt-kit et ce qui est refusé** — le fossé Rust, le CLI complet ajouté au périmètre, la journalisation qui rend applicable la règle de couche 4, la relecture du diff et ce qu'a coûté d'y croire sans mesurer, et les décisions des 2026-08-26 et 2026-08-27 |
| **`13-protocole-seance-terminal.md`** | **La séance au terminal série, exécutable** : ce que le manuel du fabricant a déjà levé (et les deux affirmations du dossier qu'il contredit), les huit blocs de commandes dans l'ordre, la fiche de relevé, **la procédure de capture sur macOS**, le format de trace que le rejeu consommera, et l'anonymisation obligatoire avant tout versement |

**Ordre de lecture recommandé :** `09` d'abord (l'architecture retenue), puis **`11`** (ce que le logiciel doit savoir d'un véhicule, et d'où ça vient), puis `10` (la conception du serveur), puis `04` (comment compresser). **`12` se lit juste avant d'écrire la première ligne** — c'est lui qui dit sous quelles règles le code se produit. `07` liste ce qu'il faut aller vérifier — dont l'item 4bis, le seul préalable réel au code, **dont `13` porte l'exécution**. `02` est le profil de référence n°1. `01` explique où sont les frontières d'accès aux données. `03`, `05` et `06` sont de la documentation de référence. `08` est à ignorer.
...
> - **La relecture du diff reste humaine**, avec **CodeRabbit** en plus. Le montage à trois relecteurs agents envisagé le 2026-08-26 — qui aurait supprimé la porte humaine — a été **renversé le 2026-08-27** : Gemini n'a jamais été installé sur ce dépôt et `claude-review` coûtait de l'ordre de 0,60 $US par tour. Sur un projet personnel, un seul relecteur agent se justifie, et il ne remplace pas une lecture (`12` §6.3). `cargo-deny` est une porte de dépendances, pas un relecteur, et dort tant qu'il n'y a pas de `Cargo.toml`.
