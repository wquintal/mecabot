...
> - **Il n'y a pas de porte de relecture humaine sur le diff.** Elle est remplacée par **trois relecteurs de code** — Gemini, CodeRabbit, Claude. `cargo-deny` s'y ajoute comme porte de dépendances, mais ce n'est pas un relecteur et il dort tant qu'il n'y a pas de `Cargo.toml`. La substitution n'est valide que tant que les trois tournent réellement, et trois est le plancher (`12` §6.3).
